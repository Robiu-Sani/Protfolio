...
export default function PreactiesCard({ data }) {
  ...
  return (
    <div className="w-full animationTimeline border overflow-hidden rounded-md relative shadow-md min-h-[100px] group">
      ...
      <div className="w-full bg-gray-200 animationTimeline flex justify-center items-center p-1">
        <small className="font-bold animationTimeline">{name}</small>
      </div>
      ...
      <div className="absolute  bottom-0 left-0 right-0 bg-white h-full bg-opacity-80 flex justify-center gap-5 items-center p-2 transform translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-in-out">
        ...
        >
          <FaGithub className="mr-1 animationTimeline" /> GitHub
        </a>
        <a
          href={LiveLink}
          target="_blank"
        ...
        >
          <FaExternalLinkAlt className="mr-1 animationTimeline" /> Live Link
        </a>
      </div>
    </div>
  );
